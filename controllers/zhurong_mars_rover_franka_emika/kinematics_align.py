...
def print_pinocchio_body_info():
    ...
    for i in range(len(ph_model.names)):
        ...
        print(f"body {i:2d}: {name:20s}:  joint pos = {joint_pos}, visual pos = {visual_pos}, collision pos = {collision_pos}")
...
def pinocchio_forward_kinematics(q):
    ...
    pinocchio.updateGeometryPlacements(ph_model, ph_data, ph_collision_model, ph_collision_data)
    pinocchio.updateGeometryPlacements(ph_model, ph_data, ph_visual_model, ph_visual_data)
...
class CustomViewer:
    ...
    def run_loop(self):
        ...
        while self.is_running():
            ...
            pinocchio_forward_kinematics(mujoco_q_to_pinocchio_q(self.initial_mj_q[:36]))
